...
- HTML report file exports stream rendered output. Desktop reports run on a background worker after the crawl stops and publish atomically, preserving existing files on failure. Report inputs still hydrate full records and up to one million link edges; larger edge sets return an explicit error. The legacy string API remains available.
