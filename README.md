...
- Frontier checkpoints include waiting and in-flight URLs. Dispatch reuses that saved state without a write. Ordinary completions update only completed/new queue entries, new seen keys, sitemap flags and the crawled counter in one SQLite transaction. Memory updates its saved state in place. Startup, worker errors and Stop retain full snapshots; see the [measurements](docs/BENCHMARKS.md#incremental-frontier-checkpoints).
- SQLite reuses queue/seen INSERT statements within each complete checkpoint transaction. Local checkpoint persistence took 66–71% less time in the [matched workload](docs/BENCHMARKS.md#sqlite-frontier-checkpoint-inserts); startup/Stop snapshot copying and summary scans still grow with the crawl.
