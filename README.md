...
- Archive exports retain one full Memory record snapshot and hydrate the full frontier; Memory query work and archive import are still unbounded. Comparison parses and stages archive records once before serving bounded SQLite pages; that initial archive/Memory record allocation still scales with the whole crawl. Counts changing during export trigger a retry error; same-count edits from an external database connection are not a transactional snapshot guarantee.
