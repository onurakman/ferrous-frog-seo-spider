...
- Responses without a `Content-Type` header are parsed as HTML only when the body starts with an HTML document; the grid then shows `text/html (inferred)` so header-derived types stay distinguishable. Failed pages, incomplete downloads and non-HTML resources do not inflate missing metadata audits; robots exclusions are separate from broken requests in results and reports.
...
- **Pagination > Next URL to Error / Previous URL to Error** lists complete, successful HTML source rows when any captured `rel="next"` or `rel="prev"` target has a known HTTP or connection failure. The grid exposes the first declared target and full target arrays, with separate source counts including List occurrences. Pending, uncrawled and robots-blocked targets are not failures; successful redirects retain their route evidence. SQLite shares the canonical alias cache and returns bounded result pages.
- **Next URL Loops / Previous URL Loops** checks each direction independently, including self-links and paths entering a cycle through observed complete HTML pages. After a chosen declaration, traversal stops at another page with multiple distinct targets, so links from separate sequences are not combined into a claimed loop. Unknown targets also end the path.
- **Next URL Non-Reciprocal / Previous URL Non-Reciprocal** warns when an observed complete target has no opposite relation or its measured opposite targets all return to other known complete pages. Any return to the declaring source, including a proven redirect alias, satisfies that declaration; unresolved return alternatives remain unclassified. These advisories include all captured targets, the legacy `previous` synonym, and separate List occurrences. They do not prove sequence completeness or a search-ranking requirement; documents may belong to [multiple sequences](https://html.spec.whatwg.org/multipage/links.html#sequential-link-types).
- **Multiple Pagination Declarations** inventories complete successful HTML pages with more than one valid captured `next` or `prev` declaration. It retains target URLs in declaration order, including repeats and the `previous` synonym, and shows counts in the grid and every target through a paged selected-page list. This informational view does not treat multiple sequences as an error. Invalid URL targets are excluded; older captures without the arrays show unknown rather than zero. All captured targets feed the measured diagnostics; older captures fall back to their first-target fields.
- **Pagination > Canonical to Linked Page** warns when a source canonical and a captured next/previous declaration resolve to the same different, observed complete HTML page. It checks every declaration, preserves redirect aliases and List occurrences, and skips unknown, incomplete, non-HTML and self targets. Unlinked canonical destinations are outside this advisory. The Canonicals workbook sheet includes the declaration evidence. This is a direct captured relation, not inferred multi-hop sequence membership; [Google's pagination guidance](https://developers.google.com/search/docs/specialty/ecommerce/pagination-and-incremental-page-loading) recommends separate canonical URLs and notes that it does not use next/previous link relations.
- **AMP > AMP URL to Error / AMP Canonical Return Missing** checks every captured `rel="amphtml"` URL, including declarations after the first. A failed observed target is an error; an observed complete HTML target with no canonical return, or a return to a different known page, is a warning. Source counts include separate List occurrences. Unknown, blocked, incomplete and non-HTML targets, plus malformed or unresolved return URLs, are not classified for reciprocity. **Multiple AMP Declarations** lists successfully parsed HTML pages with more than one captured declaration, including repeated targets. The first AMP URL remains available alongside ordered target evidence in the paged inspector, search, CSV/XLSX and archives. Older crawls show unmeasured target evidence until recrawled. The **AMP Target Missing Marker** view warns when an observed complete successful HTML target lacks the [required `amp` or `⚡` root attribute](https://amp.dev/documentation/guides-and-tutorials/learn/spec/amphtml/#required-markup) in its original HTTP HTML. JavaScript additions/removals do not change this source evidence. Unknown older captures remain unclassified; the marker check does not establish full AMP conformance. The nullable `amp_document` CSV/XLSX field and inspector retain present, absent and unmeasured states.
- SQLite duplicate, URL-regex and cross-record hreflang filters query bounded row windows directly. Return-link and canonical-target audits join URL aliases inside SQLite, preserve redirect/List evidence and avoid expanding shared redirect aliases into duplicate matches. Summaries and canonical graphs use a persisted record revision, so queue/seen-set writes no longer invalidate them. Progress counters refresh only changed records, retaining compact audit flags, metadata keys and duplicate-group counts rather than full row payloads. External record writes trigger a snapshot rebuild; canonical/reference graphs still invalidate on record changes. [Storage measurements](docs/BENCHMARKS.md) document the improvement and remaining scale limits.
...
- Select two saved cards and choose **Compare crawls** to compare them directly, with the older crawl as the baseline. A full viewport comparison workspace preserves the active crawl, shows complete aggregate counts, and queries 100-row virtualized pages without a total row cap. Search URLs, titles and descriptions; filter by change type or changed field; sort the results and export every matching row to CSV. Archive comparison remains available from the workbench Mode menu.
- **Changed** compares final destination, captured status, titles/descriptions and their known counts, indexability, headings/counts, canonical/count and robots directives, plus compatible content fingerprints. **Response-only** differences have separate complete counts and are hidden from rows by default; **Show response-only changes** queries the backend before pagination. Rows show change reasons, old/new descriptions, content availability and raw response hashes. Missing raw hashes are unavailable evidence, not proof that bytes changed.
- Select a comparison row to inspect captured baseline/current record fields side by side, with changed values and structured audit evidence. **Expand details** gives the selected URL the full work area; field search and **Differing values only** help focus the investigation. Comparisons prepare private record-only SQLite snapshots and classify results once; later pages, details and exports use that same snapshot. Source databases and the saved-crawl library remain untouched. First-inlink source/anchor/position values derived from link edges are unavailable in this record-only view. Matching uses the normalized original request URL and its one-based occurrence, preserving redirect aliases and repeated List inputs. Complete unique positive duplicate indexes retain their occurrence numbers; legacy groups fall back to relative List position and record ID. Reordering distinct URLs does not change their identity. Destination changes appear as **Final URL** differences; source positions, original/final URLs and occurrence evidence remain available in details and CSV.
- New crawls store a separate exact content fingerprint: whitespace-normalized HTML text excluding head/script/style/noscript/template subtrees, or complete bytes for non-HTML files. HTML fingerprints include a version, HTTP/rendered source and Content selector context; failed rendering or incomplete bodies have no comparable fingerprint. Raw response hashes and Exact Response Duplicates stay unchanged. Existing sessions/archives without fingerprints and captures with different contexts report **Content unavailable**; comparable new crawls are required. Text-node boundaries become spaces, CSS visibility is not evaluated, and changing visible timestamps still count unless excluded through Content selectors. SQLite and archives retain the fingerprints without storing HTML.
- **Settings > Extraction > Retain page evidence** independently enables client-observed response headers, raw HTML, rendered HTML and visible text; all default to off. Each body representation has a configurable 1 KiB–1 MiB limit (default 1 MiB), with UTF-8-safe truncation. Headers have separate 64 KiB / 512-field limits and credential-bearing values are redacted. These are terminal HTTP client headers after normalization, not original wire, request or browser headers. Rendered HTML requires successful DOM rendering; visible text follows Content selectors. Incomplete bodies remain unavailable, distinct from complete empty values.
- Select **Captured data** in URL details to inspect retained evidence as plain text in a 64 KiB preview. Storage and preview truncation are reported separately. Four filtered CSV exports stream retained headers/HTML/text from stopped crawls through atomic files. Evidence stays outside grid records, preserves exact List occurrences, and survives SQLite reopening and compatible archives; old crawls need recrawling to populate it.
...
- Searchable Settings navigation groups the current controls under Spider, Analysis and Workspace, with indented children, keyboard-toggleable headers and a fixed search field. Groups start collapsed when opening Settings; search expands matches and clearing it collapses the groups again. Limits, Speed and robots.txt have dedicated sections; near-duplicate similarity is under Content. Search by section or actual control labels, then press Enter or use the matching-control picker to reveal and focus the setting. Contextual help explains units, zero limits, pacing and robots policy; disabled controls remain discoverable. Narrow windows use a grouped section picker, and changing sections resets the form scroll position. Invalid numeric input reveals only its containing group.
...
- Default speed is 8 concurrent requests, a 10 requests/second limit per host, and 100 ms minimum spacing per origin. robots.txt and longer site-declared crawl delays still apply. Saved speed choices remain unchanged; use **Settings > Speed > Default preset**, then **Apply**, to adopt the new defaults.
...
- **Settings > Resources > Page references** separates Crawl and Store for canonical, hreflang, pagination, AMP, meta refresh and iframe targets. Crawl defaults to off and Store to on; effective Spider discovery forces retention. List and Exact URL never expand these references, so their disabled Crawl preferences do not override Store. HTML, rendered DOM and repeated HTTP Link canonicals follow existing request/scope/robots/resource/nofollow limits. Store keeps source occurrence, source/target URLs, reference type and effective nofollow evidence separately from audit metadata and hyperlink counts. Select a row and open **References** to inspect 100-item pages. Evidence survives SQLite reopening and archive export/import; older crawls require recrawling to populate it. HTTP-versus-DOM provenance remains pending.
...
- **Settings > Integrations > Backlinks** accepts any backlink API as an HTTP(S) URL template containing `{url}` plus an optional credential header stored in the OS credential store. Fetch and Merge queries the crawl's successful internal HTML URLs (bounded per run) and fills the Backlinks, Ref. domains and Authority grid columns and CSV/XLSX fields from the endpoint's `backlinks`, `referringDomains` and `authorityScore` JSON fields.
- **AI tab (selected URL)** classifies search intent, drafts a meta description and checks spelling/grammar with the provider configured in **Settings > AI**: Anthropic's Messages API (default model `claude-opus-5`) or any OpenAI-compatible endpoint, your own API key in the OS credential store, a per-minute request limit and a bound on the visible page text sent with each prompt. The page is fetched again at request time; page text is treated as untrusted data and results are stored with the row.
- **Settings > Integrations > Google account** stores a Desktop OAuth client ID/secret in the OS credential store and connects a Google account through the system browser (loopback redirect with PKCE). Tokens refresh automatically for Search Console and **Google Analytics 4**, whose `Fetch and Merge GA4` adds sessions, engaged sessions, key events and revenue to matching crawled URLs by host and path; the numbers appear as GA4 grid columns and CSV/XLSX fields.
- **PageSpeed bulk runs** measure the rows selected in the grid (Ctrl/Cmd-click, up to 500) one after another with the chosen device and Lighthouse categories. Quota responses are retried with backoff, rows already measured for that device are skipped, progress and a final summary appear as notices, and the run is cancellable. Lab and field results are shown in the PageSpeed (lab) and Field data (CrUX) grid columns and exported as CSV/XLSX fields. The unsortable CrUX cell tooltip includes all five p75 values, form factor, collection period, requested URL and UTC fetch time. CSV/XLSX retain the same attribution: `field_has_data` is blank when unmeasured, `false` for an observed no-data result and `true` when data was returned. No-data snapshots keep attribution but leave metric cells blank; measured zero values remain zero.
- **Field data (Chrome UX Report)** sits under the PageSpeed tab of the selected URL: choose phone, desktop or tablet and fetch real-user p75 Largest Contentful Paint, Interaction to Next Paint, Cumulative Layout Shift, First Contentful Paint and Time to First Byte with the collection period. It uses the saved Google API key (the Chrome UX Report API must be enabled for that key), reports URLs without field data explicitly, keeps lab and field results separate, and stores the latest snapshot per row in the crawl database and archives.
- **Bulk field data** fetches up to 500 selected rows with the chosen form factor, a progress indicator and cancellation that also works outside the PageSpeed tab. Attempts run one at a time, at least 500 ms apart; HTTP 429/503 responses use bounded retry backoff. The [CrUX project quota](https://developer.chrome.com/docs/crux/api#rate-limits) is shared with other clients. Saved results for the same requested URL and form factor are skipped, including measured no-data results; use the single-URL action to refresh an existing snapshot. Completed rows remain saved after cancellation or later failures, and repeated List URLs retain their separate record identities. Missing-key or locked-keyring errors stop the batch immediately.
- **Settings > HTTP headers > HTTP authentication** saves a Basic username/password in the OS credential store and, when the configuration enables it, sends them only to the starting origin (pages, robots.txt, sitemaps and same-origin redirects) as Basic authentication, or answers a `Digest` challenge (MD5, `qop=auth`) once per request with the same credentials. Profiles and archives store only the enabled flag; a missing or unreadable credential blocks Start with an explanation. Browser rendering does not use these credentials. The CLI reads `FERROUS_FROG_BASIC_USER` and `FERROUS_FROG_BASIC_PASSWORD`.
- **Settings > HTTP headers > Form login** saves a username/password in the OS credential store and, when enabled, posts them once to the login URL using the configured field names plus optional extra fields before the crawl starts. Cookies set by that response stay in the crawl's cookie jar; a failed login stops the crawl with the HTTP status. The CLI reads `FERROUS_FROG_FORM_USER` and `FERROUS_FROG_FORM_PASSWORD`.
- **Settings > Automation > Schedule** starts a fresh crawl of the configured start URL once at a chosen time or repeatedly every 5 minutes to 7 days, only while the app is open and idle. Progress is remembered across restarts and reset when the schedule changes; a missed one-off time runs when the app next opens. For unattended runs use the CLI with the operating system's scheduler.
- **Settings > Automation** runs after each crawl: an optional export preset (`basic`, `audit`, `full`) written to a new folder under the exports directory, an optional webhook that receives a JSON summary (status, counts, summary, export paths) for finished and failed crawls, and an optional desktop notification. Outcomes and failures appear as in-app notices without changing the crawl result.
- **Settings > Thresholds** edits the title and meta description character/pixel limits, H1/H2 lengths, the HTML report's large-image size, the thin-content word count and the minimum visible-text ratio behind the Content tab's Thin Content and Low Text Ratio views. Limits apply immediately to the grid views, analysis issues and HTML report without re-crawling; a reset restores the defaults. Alt-text length and oversized-image flags are decided during the crawl and stay fixed.
- **Settings > Scope > CDN hosts** lists hosts, optionally with a path prefix, that count as internal for link classification, record classification, scope and resource rules. Entries never enable external crawling: a CDN image is requested only when image crawling is on. Invalid entries are rejected before saving.
- **Settings > Limits** adds optional discovery limits: max folder depth (directory segments), max URL length (characters) and max links queued per page. Zero means unlimited. Limits apply to discovered links, sitemap entries and resumed queues; the seed is always crawled and link edges are retained beyond the per-page cap.
- **Settings > Resources** separates Crawl and Store choices for HTML pages, images, CSS, JavaScript, other files and external URLs. Crawl requests discovered URLs of that type; Store keeps their link edges, outlink/inlink counts and image references without requesting them. Store defaults to on, crawled types are always stored, and canonical/hreflang/pagination/AMP metadata retention is not affected. External fetching is off by default. Enabling it checks linked external URLs and follows their redirects without expanding links from external pages; include/exclude rules and robots still apply. Exact URL never expands external links, and Spider sitemap seeds remain internal. Turning external fetching off removes external pending entries when resuming.
...
- HTML parsing for titles, meta descriptions, meta keywords, estimated title/meta pixel widths, H1/H2, canonicals, directives, images, mobile viewport, AMP, pagination, hreflang alternate URLs, JSON-LD syntax and basic schema.org/rich-result fields, Open Graph, Twitter Cards, deprecated HTML tags, duplicate id attributes, links, and image/stylesheet/script resources.
...
- **Markup > Missing HTML Doctype** flags complete successful HTML whose original HTTP source has no parsed HTML doctype. The grid, technical inspector and CSV/XLSX distinguish present, missing and unmeasured legacy evidence. Rendered `outerHTML` does not replace this source measurement. This checks declaration presence, including historical declarations, rather than full HTML conformance or browser rendering mode.
...
- Checkboxes align with adjacent input controls across Settings, including Limits, Speed, robots.txt, Storage, and Rendering.
...
- Frontier checkpoints include waiting and in-flight URLs. Dispatch reuses that saved state without a write. Ordinary completions update only completed/new queue entries, new seen keys, sitemap flags and the crawled counter in one SQLite transaction. Memory updates its saved state in place. Ordinary Stop reuses the durable frontier, and worker errors remove only their pending key. Startup and interrupted linked-sitemap discovery retain full snapshots; see the [checkpoint](docs/BENCHMARKS.md#incremental-frontier-checkpoints) and [Stop measurements](docs/BENCHMARKS.md#stopping-without-replacing-the-durable-frontier).
- SQLite reuses queue/seen INSERT statements within each complete checkpoint transaction. Local checkpoint persistence took 66–71% less time in the [matched workload](docs/BENCHMARKS.md#sqlite-frontier-checkpoint-inserts); startup/interrupted-discovery snapshot copying, scheduler cleanup and summary scans still grow with the crawl.
...
- **Export > Crawl Archive** writes the whole stopped or completed crawl to `.ffcrawl.json`, independently of grid filters. The background worker writes schema 1 directly to an atomic file, preserving record order, List occurrences, links, images, reference evidence, retained page data and complete resume state. Records stream in backend order: Memory clones one record at a time; SQLite decodes batches of at most 256 in one read transaction, including first-inlink evidence. Link/image/reference collections use 10,000-item pages; retained page data uses separate 16-item pages. The complete resume queue and seen keys stream one row at a time from a single SQLite read transaction; Memory borrows its saved frontier without cloning it. The former one-million-link/image cutoff is removed. Failures preserve existing files and release the workspace.
- Schema-1 archive imports decode records, links, images, references, captures and frontier data incrementally into a private SQLite database on a background worker. Only a fully validated database becomes a saved session and replaces the active workspace. Sparse IDs, List occurrences, captured link metadata, the initial seed and queued order survive; duplicate identities, invalid captures, malformed sections and trailing input fail without changing the active crawl. Older archives may omit optional reference/capture fields. Evidence/frontier batches contain at most 256 rows; individual fields and records can still be large.
- Memory grid and link queries filter and sort borrowed records/edges, then copy only the requested window. Summary queries also borrow the records; anchor reports borrow their inputs while retaining per-group aggregates. First-inlink search, sorting and returned evidence use the maintained index. Candidate lists, audit indexes, text matching and aggregate scans still grow with the crawl; each query holds a read lock until completion, so concurrent Memory writes wait. SQLite remains the default desktop backend.
- Memory indexes link endpoints when edges are appended, so a record update refreshes only related edge status/depth evidence. Redirect aliases, repeated List occurrences and unknown statuses retain their previous behavior. The indexes retain URL keys and edge positions proportional to the graph; whole-crawl progress and query scans remain.
- Archive export and comparison preparation stream current Memory/in-memory SQLite records instead of cloning the entire record collection. Memory maintains and reuses a first-inlink index proportional to distinct link targets, and SQLite sorting/query work can grow with the crawl. The comparison workspace streams archive records into private SQLite staging before serving bounded pages, preserving original IDs and List occurrences; saved SQLite sources keep their direct SQL copy. The legacy archive comparison command remains buffered. Count changes detected between pages of link/image/reference/capture collections trigger a retry error; the archive as a whole is not a transactional snapshot against external database writes. Its record and resume-state sections each have their own consistent read snapshot.
- HTML report file exports stream rendered output and scan every retained link edge, keeping exact counts and at most 50 sample rows per section. Desktop reports and standard CSV/XML file exports run on a background worker after the crawl stops and publish atomically, preserving existing files on failure. All ten summary sections and global duplicate counts remain. Records are read in bounded batches; compact duplicate-text and failed-URL indexes still grow with the crawl. Count/content changes between the two record passes abort before output; record and edge passes are separate snapshots. CLI and automatic link CSV exports also stream every edge without a one-million-row ceiling. Legacy HTML and whole-link CSV string commands remain available, retain their complete output strings, and now require the crawl to stop or complete before exporting.
...
`cargo run -p ferrous-frog-cli -- crawl https://example.com --max-urls 500 --preset audit --output ./exports` runs a headless Spider crawl and writes CSV, audit workbook and HTML report into the folder; `list <urls...|file>` crawls explicit URLs, `--profile <name>` loads a profile saved by the desktop app (`profiles` lists them, `--app-data` overrides the platform data directory), `--config <file.json>` loads an exported configuration, and `--database <file>` keeps the crawl in SQLite. Progress goes to stderr and a JSON summary to stdout; the exit code is non-zero on validation or crawl failures. Export presets are `basic` (CSV), `audit` (CSV, workbook, HTML) and `full` (all formats). CSV, XLSX, sitemap and redirect CSV write from the storage record stream; XLSX uses temporary worksheet storage. Each CLI/automatic export replaces its destination only after a successful write and flush; failed exports preserve the previous file. Bundles publish one file at a time, so earlier successful files remain if a later format fails. Excel limits each cell to 32,767 characters and each worksheet to 1,048,575 data rows; exceeding those limits fails explicitly rather than truncating evidence. Use CSV for larger values or datasets.
...
[Storage and local crawler measurements](docs/BENCHMARKS.md) include a configurable local HTTP fixture (1,000 pages by default), with physical-disk runs through 10,000 pages and concurrent bounded grid/link/recovery queries. It checks cycles, duplicate URLs, planted failures, robots exclusions and stop/reopen/resume in Memory and SQLite. The paired 5,000-page measurements document first-inlink indexing; the 10,000-page follow-up covers borrowed query windows and indexed edge-status updates. This ignored release-profile test is separate from normal CI and the one-million-record storage benchmark; actual desktop responsiveness and larger/denser live crawls remain unmeasured.
...
`make test-rendering` runs real-Chrome fixtures one at a time and checks shared robots rules, sustained request pacing, configured User-Agent, cross-site frames, workers, popups, redirects and Pause/Resume/Stop. Browser fixtures use the normal render timeout; virtual-time tests separately verify that paused work is discarded when its deadline expires or it fails, Resume starts fresh work, and Stop cancels it. Headless policy tests verify exact request-admission intervals and concurrent callers; browser timing checks exclude startup and allow delivery jitter across the request window. Set `CHROME` to an installed browser path; its native sandbox must work. Each rendered page uses an isolated temporary profile, and cancellation drops its browser process and event handler. Pause abandons unfinished browser renders; Resume reloads those pages for rendering with fresh command deadlines. It keeps the crawl row pending and does not convert a long pause into a raw-HTML fallback. HTTP requests use the crawler policy; WebSocket/WebRTC traffic and full browser network diagnostics are not covered by these checks. An actual render timeout or browser failure falls back to the fetched HTML with the error attached to its row.

`make test-native` provides a separate Linux harness for the built application, using WebKitGTK/WebDriver, an isolated virtual display and temporary crawl storage. See [native testing](docs/NATIVE_TESTING.md) for setup and validation status.
