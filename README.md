...
- **Export > Crawl Archive** writes the whole stopped or completed crawl to `.ffcrawl.json`, independently of grid filters. The background worker writes schema 1 directly to an atomic file, preserving record order, List occurrences, links, images, reference evidence, retained page data and complete resume state. SQLite records and all link/image/reference collections use 10,000-item pages; retained page data uses separate 16-item pages. The former one-million-link/image cutoff is removed. Failures preserve existing files and release the workspace.
...
- Archive exports retain one full Memory record snapshot and hydrate the full frontier; Memory query work remains unbounded. The comparison workspace streams archive records into private SQLite staging before serving bounded pages, preserving original IDs and List occurrences. Current Memory/in-memory SQLite sources still hydrate their record snapshot, and the legacy archive comparison command remains buffered. Counts changing during export trigger a retry error; same-count edits from an external database connection are not a transactional snapshot guarantee.
- HTML report file exports stream rendered output and scan every retained link edge, keeping exact counts and at most 50 sample rows per section. Desktop reports run on a background worker after the crawl stops and publish atomically, preserving existing files on failure. All ten summary sections and full record/global duplicate context remain; record inputs still hydrate in full. CLI and automatic link CSV exports also stream every edge without a one-million-row ceiling. Legacy HTML and whole-link CSV string commands remain available, retain their complete output strings, and now require the crawl to stop or complete before exporting.
