...
| UI-05 | Settings navigation | Available: seventeen sections include dedicated Limits, Speed and robots.txt controls; near-duplicate similarity belongs to Content. Search matches section keywords and actual control labels, with a grouped control picker and Enter to reveal, scroll to and focus the first available match. Unavailable controls remain listed; search never indexes saved field values. Initially collapsed groups, breadcrumbs, keyboard navigation, a narrow-window section picker and scroll reset remain. Contextual help explains units, zero limits, pacing, presets, robots policy and the HTTP/browser download boundary without changing saved preferences. |
...
| CR-03 | Alternate-link matrix | Partial: canonical, hreflang, pagination and AMP have independent default-off Spider Crawl and default-on Store choices. Effective Crawl forces retention; List/Exact URL preserve the disabled Crawl preferences while keeping Store editable. Stored references retain source URL, exact source storage occurrence, target, type and nofollow evidence; the References inspector queries windows of 100 with paging, live updates and stale-selection/session protection. Audit metadata and hyperlink counts remain independent. Existing request/scope/robots/resource/nofollow/depth rules still govern discovery. Generic/mobile alternates and HTTP-header/raw-HTML/rendered-DOM provenance (including intermediate redirects) remain missing. |
| CR-04 | Embedded navigation | Available: default-off meta-refresh and iframe Crawl choices follow the reference-link rules (scope, robots, resources, nofollow, depth; not in List/Exact URL), with separate default-on Store choices. Target/source/type/nofollow evidence is available in the paged References inspector without requesting the target or changing hyperlink counts. Saved crawls without reference evidence need recrawling. |
...
| EX-03 | Response details | Partial: timings, hashes, size, selected header flags and opt-in client-observed terminal response headers are available, with bounded storage, redaction, selected inspection and filtered CSV export. Original wire/request/browser headers and separate browser download limits remain missing. |
...
| EX-06 | HTML retention | Available: independent default-off raw HTML, rendered HTML and scoped visible-text retention, with configurable 1 KiB–1 MiB per representation, UTF-8-safe truncation, exact occurrence storage, cleanup and archive compatibility. Selected plain-text previews cap at 64 KiB; filtered CSV exports stream retained text. Failed/incomplete captures remain unavailable, distinct from empty content. Rendered source requires successful DOM rendering. |
...
| AD-07 | HTML validation | Partial: duplicate IDs, deprecated tags and missing HTML doctype, with nullable original-HTTP declaration evidence in the grid, inspector and CSV/XLSX. Doctype presence is not conformance or rendering-mode validation. Add configurable validation with contextual evidence. |
...
| API-04 | Field performance | Available: selected-URL and bulk selected-row CrUX fetching (up to 500), with phone/desktop/tablet, cancellable paced requests, bounded quota retries and URL/form-factor resume including no-data results. Latest snapshots persist per exact record beside Lighthouse data. The unsortable grid tooltip and CSV/XLSX include p75 LCP/INP/CLS/FCP/TTFB and URL/form-factor/collection-period/fetch-time attribution, with distinct unmeasured/no-data states and preserved zero values. No result history; bulk resume retains existing matching snapshots. |
...
| API-07 | Optional AI providers | Available: configurable Anthropic/OpenAI-compatible endpoints, model selection, OS-keyring credentials, input/request limits and selected-URL intent/meta-description/spelling tasks. Saved single-crawl and follow-up reports reuse these settings and credentials with explicit data/budget previews. |
| API-08 | AI audit reporting | Available: saved English/Turkish reports with frozen scope/thresholds, 16 page rules and broken-link evidence; complete affected-URL search/status filtering/paging and portable HTML/CSV packages. Optional single-crawl/follow-up AI provides validated explanations and overviews with explicit budgets, cancellation/resume, usage and preserved generation provenance. Computed comparison states retain missing/incompatible observations; unsupported checks stay unmeasured. See [AI-R01–AI-R06](../ROADMAP.md#ai-audit-reporting), the [delivery plan](superpowers/plans/2026-09-14-ai-audit-reporting.md) and [scale measurements](BENCHMARKS.md#frozen-audit-reports-100000-pages-and-1000001-links). |
...
| AN-07 | Pagination analysis | Partial: captured next/prev fields (including legacy previous), target errors, direction-specific loops and advisory non-reciprocal warnings, sortable targets and typed issues with Memory/SQLite parity. Diagnostics inspect all captured declarations, with a multiple-declaration inventory and known complete HTML target evidence; unknown returns remain unclassified. A direct canonical-to-linked-page advisory exposes captured relation/canonical conflicts. Unlinked sequence members and multi-hop sequence consistency checks remain. |
...
| ID | Control/workflow | Current state and remaining work |
| --- | --- | --- |
| OP-01 | Storage | Available: automatic SQLite desktop sessions, centered startup launcher, initially closed bottom history panel with a saved disclosure preference, configuration restore, two-card comparison, location, archives and frontier recovery. Memory remains a headless backend. Comparison distinguishes tracked SEO fields, compatible normalized HTML text and file content from default-hidden response-only differences; both saved-session and archive routes query the selected filter in the backend. A full viewport comparison workspace offers complete counts, bounded virtualized pages without a total row cap, search/change/field filters, sorting, captured baseline/current record details and filtered CSV export. Private record snapshots preserve the active crawl; archives parse once before paging. Normalized original request URL plus one-based occurrence preserves redirect aliases and repeated List inputs across saved/archive/compatibility routes; destination changes, source positions and occurrence evidence appear in details and CSV. Complete unique positive duplicate indexes take precedence over relative List-position/ID fallback. Archive and current Memory/in-memory SQLite records stream into private staging; Memory first-inlink indexing and SQLite sorting still grow with input size. Older or incompatible text captures require comparable new crawls. See [ROADMAP](../ROADMAP.md). |
