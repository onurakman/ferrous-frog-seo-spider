...
| API-07 | Optional AI providers | Partial: configurable Anthropic/OpenAI-compatible endpoints, model selection, OS-keyring credentials, input/request limits and selected-URL intent/meta-description/spelling tasks exist. Sitewide reporting remains separate; reuse this configuration for the planned report workflow rather than adding a plugin runtime. |
| API-08 | AI audit reporting | Partial, under implementation: saved report title/language/scope and deterministic evidence snapshots; optional AI explanations and data/request budgets; saved deterministic findings with complete affected-URL search/paging, evidence counts and portable HTML/CSV reports. Include follow-up comparison states, coverage, cancellation/resume and unavailable-provider handling. See [AI-R01–AI-R06](../ROADMAP.md#ai-audit-reporting) and the [delivery plan](superpowers/plans/2026-09-14-ai-audit-reporting.md). |
