...
The expanded smoke passed on 2026-09-20 with an embedded-assets debug build from the working tree based on `6299b66`, including incremental SQLite progress summaries and the new pagination, AMP marker and HTML doctype audits. All three filters returned exactly their affected source before and after a process restart; the existing polite crawl, frozen report, export and quit checks also passed. The executable SHA-256 was `d0826d7eb314659586daf3d5b6288936118458e5fcc5eb672f3625a3e36d277e`; retained local artifacts are `/tmp/ferrous-native-smoke-CzKXif`. This Linux debug run does not extend installer or other-platform coverage. Bulk CrUX selection/progress/cancellation and current-query refresh pass the browser IPC fixtures; backend virtual-time tests cover pacing, quota retry, resume, credential failures and guarded persistence. Those checks do not access live Google services or user credentials.
