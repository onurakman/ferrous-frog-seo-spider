...
- [x] Exercise the Linux embedded-assets desktop through crawl, SQLite reopen, saved audit-report creation/reopen/export and clean quit. Linux installer payloads also passed from private extraction; the normal packaging route is being verified separately. Windows/macOS, host installation and signing remain explicit roadmap gaps.
