...
- [ ] (Partial) Full audit-rule set for all technical SEO tabs. Core row audits, canonical target/chains/loops, pagination target errors/loops and captured-relation reciprocity warnings, AMP target errors and canonical-return warnings, hreflang checks, structured-data checks, security checks, sitemap orphaning, near-duplicates, and HTML validation signals exist; meta keywords are captured, searchable, sortable and exported; remaining gaps include complete pagination sequences and multiple relations, AMP markup and multiple declarations, and richer HTML validation.
