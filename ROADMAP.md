...
- [x] Query recovery counts without hydrating the complete queue/seen checkpoint, preserving Memory/SQLite recovery semantics and custom-backend compatibility. Native reads use a background worker; the [isolated one-million-seen SQLite workload](docs/BENCHMARKS.md#recovery-counts-without-frontier-hydration) falls from a 72.260 ms median to 1.158 ms. Scheduler startup/Stop snapshots and broader active-crawl performance remain open.
...
- [ ] (Partial) Streaming exports for every bulk report. Grid, selected/queued URL, image-alt and link/redirect/sitemap-validation CSV plus XML sitemap write incrementally. Filtered XLSX and the audit workbook use bounded queries and temporary worksheet files; graph JSON and node/edge CSV serialize bounded snapshots directly. Archives stream JSON with SQLite record paging and complete edge/image/reference/capture paging, while retaining Memory snapshots and full frontier hydration. HTML file reports stream template output, with atomic desktop publication and full record/link input snapshots (an explicit one-million-edge ceiling); the legacy string API and archive import still retain full buffers. Comparison stages archives once before bounded queries; initial archive/Memory record hydration remains unbounded.
